// File: E:\quintos_ai\components\marketing\contact\ContactForm.tsx
...
function ContactFormInner() {
  ...
  const [copiedInquiryId, setCopiedInquiryId] = useState(false);
  ...
  const validate = (values: FormState): FormErrors => {
    ...
    if (!values.message.trim()) {
      ...
      errs.message = "Message must be at least 10 characters to provide sufficient technical context";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatusMessage("Transmitting inquiry to Quintos AI backend...");
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        ...
        setStatusMessage(
          data.error?.message ||
            "Unable to submit your inquiry. Please try again or email us directly at contact.quintosresearch@gmail.com."
        );
        return;
      }
      ...
      setStatusMessage(
        "Thank you. Your inquiry has been received and recorded in our system. Our engineering and research team will review your submitted details."
      );
    } catch (err) {
      console.error("[Contact Form Submission Error]:", err);
      setSubmissionPhase("error");
      setStatusMessage(
        "Network connection error. Please verify your internet connection or email us directly at contact.quintosresearch@gmail.com."
      );
    }
  };
  ...
  const handleCopyInquiryId = () => {
    ...
    setCopiedInquiryId(true);
    setTimeout(() => setCopiedInquiryId(false), 2000);
  };
  ...
  const handleCopySummary = () => {
    const summaryText =
      ...
      `--------------------------\n` +
      (submittedInquiryId ? `Reference ID: ${submittedInquiryId}\n` : "") +
      `Category: ${activeCategoryConfig.title}\n` +
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            {submissionPhase === "success" && (
              <div
                role="alert"
                aria-live="polite"
                className="rounded-xl border border-emerald-200 bg-emerald-50/95 p-5 text-xs text-emerald-950 space-y-3 animate-in fade-in"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {submittedInquiryId && (
                      <div className="mt-2.5 inline-flex items-center gap-2 rounded-lg border border-emerald-300 bg-white/80 px-3 py-1.5 font-mono text-[11px] text-emerald-900">
                        <span>Ref ID: <strong>{submittedInquiryId}</strong></span>
                        <button
                          type="button"
            ...
            {submissionPhase === "error" && statusMessage && (
              <div
                role="alert"
                aria-live="assertive"
                className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs text-red-900 flex items-start gap-3 animate-in fade-in"
              >
            ...
            <div className="pt-2 flex items-center justify-between">
              ...
              >
                {isSubmitting ? "Transmitting Inquiry..." : "Submit Technical Inquiry"}
              </Button>
