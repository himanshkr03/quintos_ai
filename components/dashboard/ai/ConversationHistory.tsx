// File: E:\quintos_ai\components\dashboard\ai\ConversationHistory.tsx
...
export default function ConversationHistory({
  ...
  const content = (
    <div className="flex h-full w-72 sm:w-80 flex-col justify-between border-r border-slate-200/80 bg-slate-50/50 p-4 text-xs">
      <div>
        ...
        <div className="flex items-center justify-between gap-2 pb-3 border-b border-slate-200">
          <div>
            ...
            <span className="text-[10px] text-slate-400 font-mono">Session History</span>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {onClose && (
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl p-1.5 text-slate-500 hover:bg-slate-200 lg:hidden"
        ...
        <div className="mt-3 space-y-4 max-h-[460px] overflow-y-auto pr-1">
          ...
          {sessions.length === 0 && (
            <div className="p-4 text-center text-slate-400 font-mono text-[11px]">
              No active research sessions recorded.
            </div>
          )}
        </div>
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <div className="relative z-50 h-full max-w-[85vw]">{content}</div>
        </div>
      )}
    </>
  );
