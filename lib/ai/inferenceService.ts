...
import { getInferenceProvider } from "./providers";
 ...
 * Coordinates server-side prompt construction, provider adapter selection, and streaming.
 */
export class ProductionInferenceService {
  async *streamInference(request: InferenceRequest): AsyncIterable<InferenceChunk> {
    const config = getAIConfig();

    // Attach server-controlled system prompt if not specified
    const systemPrompt =
      request.systemPrompt || buildSystemPrompt(request.modelId, request.contextId);
...
export const productionInferenceService = new ProductionInferenceService();

/**
 * Demonstration Mode Simulated Inference Service.
 * Preserved for offline development and UI testing when AI_DEMO_MODE=true.
 */
export class DemoInferenceService implements InferenceService {
  ...
  private generateDemonstrationContent(
    ...
    if (
      ...
      lowerPrompt.includes("quantum") ||
      lowerPrompt.includes("vqe") ||
...
- **Verification Guarantee:** Measured ground-state eigenvalues can be checked against exact classical diagonalization for benchmark systems.`;
    ...
    if (
      ...
      lowerPrompt.includes("segmentation") ||
      lowerPrompt.includes("vision") ||
      lowerPrompt.includes("biomedical") ||
      lowerPrompt.includes("ct")
    ) {
...
// Singleton instance for demonstration mode
